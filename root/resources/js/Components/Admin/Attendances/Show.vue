...
const form = reactive({
    ...
    clock_out: dayjs(props.stampData.data[0]?.clock_out).isValid()
                ...
                : null,
    break_minutes : props.stampData.data[0]?.break_minutes ?? null,
    vacation_type : props.stampData.data[0]?.vacation.vacation_type ?? null,
    reason : props.stampData.data[0]?.attendance_status?.reason ?? '',
    status : props.stampData.data[0]?.attendance_status?.status ?? null,
});
...
const stamp = id => {
    if (id) {
        router.post(route('admin.attendances.update', id), form, {
            onSuccess: () => {
                emit('close');
            },
            onError: () => {
        ...
        router.post(route('admin.attendances.store'), form, {
            onSuccess: () => {
                emit('close');
            },
            onError: () => {
